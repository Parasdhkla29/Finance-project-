  ...
  return (
    <div className="p-4 lg:p-6 space-y-4 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" icon={<span>+</span>} onClick={() => { setEditing(undefined); setShowForm(true); }}>Add Budget</Button>
      </div>

      {active.length === 0 ? (
        <Card>
          <p className="text-slate-500 text-sm text-center py-8">
            No budgets set. Create one to track your spending limits.
          </p>
        </Card>
      ) : (
          ...
          {active.map((budget) => {
            ...
            return (
              <Card key={budget.id}>
                ...
                {/* Progress bar */}
                <div className="w-full bg-slate-700 rounded-full h-2 mb-3" role="progressbar" aria-valuenow={Math.min(pct, 100)} aria-valuemin={0} aria-valuemax={100}>
                  <div
                    className={`h-2 rounded-full transition-all ${isOver ? 'bg-red-500' : isWarning ? 'bg-amber-500' : 'bg-emerald-500'}`}
                    style={{ width: `${Math.min(pct, 100)}%` }}
                  />
                </div>
                <p className="text-xs text-slate-500 mb-3">{pct.toFixed(0)}% of budget used</p>
