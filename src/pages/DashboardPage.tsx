...
function TrendBadge({ current, prev, inverse = false }: { current: number; prev: number; inverse?: boolean }) {
  ...
  if (Math.abs(pct) < 1) return <span className="text-xs text-slate-400">—</span>;
  const isUp = pct > 0;
  const isGood = inverse ? !isUp : isUp;
  return (
    <span className={`inline-flex items-center text-xs font-medium ${isGood ? 'text-emerald-600' : 'text-red-600'}`}>
...
function InsightBanner({ insight }: { insight: Insight }) {
  ...
  return (
    <div className="flex items-start gap-3 bg-white border border-slate-200 rounded-xl p-3">
      ...
      {insight.actionRoute && (
        <Link to={insight.actionRoute} className="shrink-0 text-xs text-sky-600 hover:text-sky-500 transition-colors">
          View →
        </Link>
      )}
    </div>
...
function KpiCard({
  ...
  return (
    <button
      onClick={() => navigate(to)}
      className="text-left w-full bg-white hover:bg-slate-100/70 active:bg-slate-100 border border-slate-200 rounded-xl p-4 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:ring-offset-2 focus-visible:ring-offset-white cursor-pointer"
    ...
    <div className="p-4 lg:p-6 space-y-4 max-w-6xl mx-auto">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-4">
        ...
        <div className={`flex items-center gap-3 px-3 py-2.5 -mx-1 rounded-xl border ${
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-slate-800">{format(now, 'MMMM yyyy')}</p>
            <p className={`text-xs mt-0.5 ${
              healthColor === 'emerald' ? 'text-emerald-600' : healthColor === 'amber' ? 'text-amber-600' : 'text-red-600'
            }`}>{healthMsg}</p>
          </div>
          <div className="text-right shrink-0">
            <p className="text-xs font-medium text-slate-500">Day {dayOfMonth}/{daysInMonth}</p>
            <div className="flex items-center gap-1.5 mt-1">
              <div className="w-16 h-1 bg-slate-100 rounded-full overflow-hidden">
                <div className="h-full bg-slate-500 rounded-full" style={{ width: `${monthPct}%` }} />
              </div>
              <span className="text-xs text-slate-400">{monthPct}%</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <p className="text-xs text-slate-400 uppercase tracking-widest font-medium">Net this month</p>
              ...
              <div key={acc.id} className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <span
                    ...
                    aria-hidden="true"
                  />
                  <h2 className="text-sm font-medium text-slate-500 truncate">{acc.name}</h2>
                  <span className="text-xs text-slate-400 uppercase shrink-0">{acc.currency}</span>
                </div>
                <h2 className={`text-base font-semibold shrink-0 tabular-nums ${
                  acc.net > 0 ? 'text-emerald-600' : acc.net < 0 ? 'text-red-600' : 'text-slate-400'
                }`}>
        ...
        <div>
          <div className="flex items-baseline gap-3 flex-wrap">
            <h1 className={`text-[2.75rem] font-extrabold tracking-tight leading-none ${
              totalForCurrency >= 0 ? 'text-sky-600' : 'text-red-600'
            }`}>
            ...
            <div className="flex items-center gap-1.5">
              ...
              {lastStats.net !== 0 && (
                <span className="text-xs text-slate-400">vs {format(subMonths(now, 1), 'MMM')}</span>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          <KpiCard label="Income" to="/transactions">
            <p className="text-lg font-bold text-emerald-600 mt-1 leading-none">{formatCurrency(stats.income, currency)}</p>
            <div className="mt-1.5"><TrendBadge current={stats.income} prev={lastStats.income} /></div>
          </KpiCard>
          <KpiCard label="Expenses" to="/transactions">
            <p className="text-lg font-bold text-red-600 mt-1 leading-none">{formatCurrency(stats.expenses, currency)}</p>
          ...
          <KpiCard label="Savings rate" to="/insights">
            <p className={`text-lg font-bold mt-1 leading-none ${stats.savingsRate >= 20 ? 'text-emerald-600' : stats.savingsRate >= 10 ? 'text-amber-600' : 'text-red-600'}`}>
            ...
            {activeBudgets.length > 0 ? (
              <>
                <p className={`text-lg font-bold mt-1 leading-none ${
                  budgetsOnTrack === activeBudgets.length ? 'text-emerald-600'
                  : budgetsOnTrack >= activeBudgets.length / 2 ? 'text-amber-600'
                  : 'text-red-600'
                }`}>
                  {budgetsOnTrack}<span className="text-slate-400 font-normal text-base">/{activeBudgets.length}</span>
                </p>
                <p className="text-xs text-slate-400 mt-1">on track</p>
              </>
            ) : (
              <>
                <p className="text-lg font-bold text-slate-400 mt-1 leading-none">—</p>
                <p className="text-xs text-sky-600 mt-1">Set budgets</p>
              </>
            )}
          </KpiCard>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          ...
          className="text-left bg-white hover:bg-slate-100 border border-slate-200 rounded-2xl p-4 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"
          aria-label="View subscriptions"
        >
          <p className="text-xs text-slate-400 uppercase tracking-widest font-medium">Monthly subs</p>
          <p className="text-2xl font-bold text-amber-600 mt-1.5 leading-none">
            {formatCurrency(monthlySubCost, currency)}
          </p>
          <p className="text-xs text-slate-400 mt-1.5">{subscriptions.filter((s) => s.isActive).length} active subscriptions</p>
        </button>
        ...
        >
          <p className="text-xs text-slate-400 uppercase tracking-widest font-medium">You owe</p>
          <p className={`text-2xl font-bold mt-1.5 leading-none ${totalBorrowed > 0 ? 'text-red-600' : 'text-slate-400'}`}>
      ...
      <Card>
        <CardHeader>
          ...
          <Link to="/subscriptions" className="text-xs text-sky-600 hover:text-sky-500 transition-colors">Manage</Link>
        </CardHeader>
        <UpcomingBills />
      </Card>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4">
        <div className="flex items-center justify-between mb-1">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Tap W · M · Q · Y to change period · ⋯ to compare</p>
          </div>
        </div>
        <div className="mt-3">
          <NetBalanceChart />
      ...
      {activeGoals.length > 0 && (
        <button
          ...
          className="w-full text-left bg-white hover:bg-slate-100 border border-slate-200 rounded-2xl p-4 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"
          aria-label="View financial goals"
        >
          <div className="flex items-center justify-between mb-2.5">
            <div>
              <p className="text-sm font-semibold text-slate-800">Financial Goals</p>
              <p className="text-xs text-slate-400 mt-0.5">{activeGoals.length} active · {goalsProgress.toFixed(0)}% overall</p>
            </div>
      ...
      {insights.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <Link to="/insights" className="text-xs text-sky-600 hover:text-sky-500 transition-colors">See all</Link>
          </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <Link to="/transactions" className="text-xs text-sky-600 hover:text-sky-500 transition-colors">View all</Link>
        </CardHeader>
              ...
              return (
                <li key={txn.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                  ...
                  <span className={`text-sm font-semibold ml-3 shrink-0 ${
                    isScheduled ? 'text-sky-600' : txn.type === 'income' ? 'text-emerald-600' : 'text-red-600'
                  }`}>
